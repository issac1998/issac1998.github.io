...
- [CV](#cv)
  - [Raft相关优化](#raft相关优化)
- [线程绑核、独占核、GO携程绑M、M绑线程](#线程绑核独占核go携程绑mm绑线程)
- [内存屏障](#内存屏障)
  - [store buffer](#store-buffer)
  - [Invalidation queue](#invalidation-queue)
  - [如果严格执行MESI 还需要内存屏障吗？](#如果严格执行mesi-还需要内存屏障吗)
- [原子操作](#原子操作)
- [CAS操作系统是怎么做的？锁怎么加](#cas操作系统是怎么做的锁怎么加)
- [常见参数大小](#常见参数大小)
- [锁](#锁)
- [分布式限流](#分布式限流)
- [COW](#cow)
- [lua脚本怎么保证原子性](#lua脚本怎么保证原子性)
- [进程和子进程](#进程和子进程)
- [UDP QUIC](#udp-quic)
- [ETCD](#etcd)
- [GRPC](#grpc)
...
简单来说，驱动负责准备 mbuf 并将其对应的物理地址填写到描述符中，然后告知网卡。网卡的 DMA 引擎读r r r ra r r取描述符，根据其中的物理地址，直接将数据包写入或读出内存，最后更新描述符中的状态位。驱动通过轮询状态位来判断数据包是否就绪或发送完成。
...
# 线程绑核、独占核、GO携程绑M、M绑线程


# 内存屏障
MESI协议可以解决缓存一致性，但是通过总线仲裁是很慢的，不适合每个请求都完全等待，需要一定程度的out of order 
现代处理器上，处理器运行的速度比内存快很多， 有序处理器花在等待可用数据的时间里已可处理大量指令了，因此出现了乱序处理器，通过**store buffer **和**Invalidation queue**保障

但是，store buffer会带来内存一致性的问题，1. Load操作 看不到最新的store buffer的数据 2.	•写入对其他 CPU 延迟可见，同一cpu的顺序性会被破坏，其他cpu如果期待这个cpu的顺序是串行的，结果就会出错。

因此，对于第一个问题，使用store forwarding策略，再Load时先看store buffer 。对于第二个问题，就需要使用内存屏障了，sfence强制等待刷新store buffer的数据，当store buffer中还有store buffer的数据时，后续的写入操作都得放在Store buffer中。

又有新的问题，Store buffer容量有限，如果其他cpu确认invalidate过慢，会导致store buffer无限增长，因此需要 **Invalidation queue**，接受端将invalidate 命令放入本地的Invalidation queue，并直接返回invalidate成功的信息。

sfence 写屏障 确保 sfence 前的所有 Store 刷出到缓存，不允许在 sfence 后的 Store 更先可见。
lfence 保证所有 Load 在 lfence 前完成，即刷新Invalidation buffer。
mfence 全屏障

## store buffer 
当前CPU发起写操作，如果发现没有独占权，可以先将要写入的数据放在存储缓冲中，并继续运行，仿佛独占权瞬间就得到了一样。等到成功invalidate其他CPU中该地址的数据后，再把CPU0存放在store buffer中的数据推到CPU0的local cache中。并且，当前CPU在读取数据的时候应该首先检查其是否存在于存储缓冲中。

## Invalidation queue
当前CPU上收到一条消息，要使某个缓存段失效，但是此时缓存正在处理其它事情，那这个消息可能无法在当前的指令周期中得到处理，而会将其放入所谓的无效队列（Invalidation Queue）中，同时立即发送使无效应答消息。


https://cloud.tencent.com/developer/article/2298641 这篇文章非常好
## 如果严格执行MESI 还需要内存屏障吗？
CPU层面的乱序可以解决(假设仲裁都能执行正确结果)，但是还有编译器指令重排(补充)等处的乱序，可以问问AI
# 原子操作
以下文字看着莫名其妙，问问gpt
...
go 页大小4k 栈2k
堆内存不足时，向操作系统申请64M
mspan 大小是 page 的整数倍（Go 中的 page 大小为 8KB），且内部的页是连续的
# 锁
...
# 进程和子进程
父进程起子进程方式：
1. fork 拷贝mm_struct结构体，包含虚拟内存及页表
2. vfork (创建进程)或者 clone(创建线程或进程) 系统调用创建出的子进程（线程）和父进程共享虚拟内存空间
vfork创建出的进程虽然共享了虚拟内存空间，但是仍然有自己的PID。vfork 共享父栈，clone 线程不共享栈（每个线程独立栈）。它的作用只是为了专门用来做 fork + exec 优化，因为fork 在 exec 之前复制页表很浪费，需要fork → 全部复制（COW 的页表） → exec → 全部扔掉。因此vfork
创建出一个真正的、独立的、新的进程（PID 不同）。
这个新进程只是在 exec/exit 之前暂时共享父进程的地址空间，但最终它就是一个完全独立的进程。
