...
- [流程](#流程)
  ...
  - [GCSTART](#gcstart)
    ...
    - [问题](#问题)
      ...
      - [bigcache 在GC的优化](#bigcache-在gc的优化)
...
func reverse(head,tail *ListNode) (newHead,newTail *ListNode){
    oldTail:=tail
    oldHead:=head
    for head!=oldTail{
        temp:=head.next
...
func main() {
    ...
    for head!=nil{
        ...
        if cnt!=k{
            // has less than k nodes
            break
        }
        // has k nodes,[sta:head],we should reverse [sta:head-1]
        newHead,newTail:=reverse(sta,head)
        pre.next=newHead
        pre=newTail
    }
    // ans:= pre.next
